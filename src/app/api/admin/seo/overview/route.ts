...
import { getValidatedSeoConfig } from '@/lib/seo/config';
...
export async function GET(req: NextRequest) {
  ...
  try {
    const [gsc, ga4, trackedKeywords, opportunities, technicalIssues] = await Promise.all([
    ...
    const validatedConfig = getValidatedSeoConfig();
    const integrationStatuses: IntegrationServiceInfo[] = Object.values(validatedConfig).map((cfg) => ({
      id: cfg.serviceId,
      name: cfg.name,
      category: 'search_console',
      status: cfg.status,
      requiredEnvVars: [...cfg.missingVars, ...cfg.configuredVars],
      description: `Integration service for ${cfg.name}`,
      configuredEnvVarsCount: cfg.configuredVars.length,
    }));
    ...
    return NextResponse.json({ error: error.message || 'Failed to fetch SEO overview' }, { status: 500 });
  }
}
