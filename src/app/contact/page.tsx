...
function ContactForm() {
  ...
  const projectTypeParam = searchParams.get('projectType') || searchParams.get('projectTypes') || '';
  const areaParam = searchParams.get('area') || '';
  const postcodeParam = searchParams.get('postcode') || '';
  const typeParam = searchParams.get('type') || 'consultation';

  // Map incoming URL params to form initial values
  const mapProjectType = (param: string): string => {
    const lower = param.toLowerCase();
    if (lower.includes('loft')) return 'Loft Conversion';
    if (lower.includes('kitchen')) return 'Kitchen Renovation';
    if (lower.includes('bath')) return 'Bathroom Renovation';
    if (lower.includes('renovat')) return 'Full Renovation';
    if (lower.includes('garden')) return 'Garden Room';
    if (lower.includes('garage')) return 'Garage Conversion';
    if (lower.includes('extension')) return 'House Extension';
    return 'House Extension';
  };

  const initialMessage = briefParam
    ? `${briefParam}${areaParam ? ` (Estimated area: ${areaParam}m²)` : ''}`
    : '';
  ...
  const [formData, setFormData] = useState({
    ...
    projectType: mapProjectType(projectTypeParam),
    inquiryType: typeParam === 'callback' ? 'callback' : 'consultation',
  ...
  // Update initial values if search params load asynchronously
  useEffect(() => {
    if (briefParam || projectTypeParam || postcodeParam) {
      setFormData((prev) => ({
        ...prev,
        message: prev.message || (briefParam ? `${briefParam}${areaParam ? ` (Estimated area: ${areaParam}m²)` : ''}` : ''),
        projectType: prev.projectType || mapProjectType(projectTypeParam),
        postcode: prev.postcode || postcodeParam,
      }));
    }
  }, [briefParam, projectTypeParam, postcodeParam, areaParam]);
  ...
  const handleChange = (
    ...
    if (!hasStartedInteracting) {
      ...
      trackEvent('contact_form_started', { projectType: formData.projectType });
    }
    setFormData((prev) => ({
      ...prev,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      trackEvent('contact_form_submitted', {
        ...
        hasPostcode: Boolean(formData.postcode),
      });
        ...
        <Container>
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
            ...
            <div className="lg:col-span-7">
              <React.Suspense
                fallback={
                  <div className="min-h-[400px] bg-white rounded-3xl p-8 border border-slate-200 animate-pulse flex items-center justify-center text-slate-400 text-sm">
                ...
                <div className="space-y-4 text-xs sm:text-sm text-slate-600">
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <span className="text-slate-700 font-medium">{siteConfig.company.openingHours}</span>
                    </div>
                  </div>
                </div>
